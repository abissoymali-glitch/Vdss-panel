...
# --- FLASK PORT DİNLEYİCİ (Render Uyumluluğu İçin) ---
app = Flask("")
...
def get_channel_markup():
  ...
  markup.add(
      InlineKeyboardButton(
          "✅ Üye Oldum Kontrol Et", callback_data="check_sub"
      )
  )
  return markup
...
def create_admin_menu():
  markup = InlineKeyboardMarkup()
  status_text = (
      "🟢 Bakım Modu: Kapalı" if not maintenance_mode else "🔴 Bakım Modu: Açık"
  )
  ...
  markup.add(
      ...
      InlineKeyboardButton(
          "⚡ Aktiflik & Performans", callback_data="admin_active"
      ),
  )
  markup.add(
...
PACKAGE_MAPPING = {"telegram": "python-telegram-bot"}
...
def handle_start(message):
  ...
  if maintenance_mode and user_id != ADMIN_ID:
    bot.send_message(
        user_id, "🛠️ Bot şu anda bakımdadır. Lütfen daha sonra tekrar deneyin."
    )
    return
  ...
  if not check_subscription(user_id):
    bot.send_message(
        user_id,
        "⚠️ Botu kullanabilmek için öncelikle **Kanalım** adlı kanalımıza abone"
        " olmanız gerekmektedir.\n\nAbone olduktan sonra aşağıdaki **Üye Oldum"
  ...
  bot.send_message(
      user_id,
      "🤖 **VDS Çoklu Bot Paneline Hoş Geldiniz!**\n\nBu sistem üzerinden aynı"
      " anda **birden fazla Python botu** çalıştırabilirsiniz. Aşağıdaki"
...
def handle_document(message):
  ...
  if not check_subscription(user_id):
    bot.reply_to(
        message,
        "⚠️ Lütfen önce zorunlu kanalımıza abone olun!",
        reply_markup=get_channel_markup(),
    )
    return
  ...
  if user_id in cooldowns and now - cooldowns[user_id] < 5:
    bot.reply_to(
        message, "Lütfen yeni bir dosya yüklemeden önce 5 saniye bekleyin..."
    )
    return
  ...
  if not doc.file_name.endswith(".py"):
    bot.reply_to(
        message, "❌ Sadece .py uzantılı Python dosyalarını kabul ediyorum."
    )
    return
  ...
  with lock:
    ...
    local_path = os.path.join(
        SCRIPT_FOLDER, f"{user_id}_{script_id}_{doc.file_name}"
    )
    with open(local_path, "wb") as f:
      f.write(downloaded)

    bot.reply_to(
        message, "🔄 Gerekli kütüphaneler kontrol ediliyor ve yükleniyor..."
    )
    ...
    try:
      ...
      bot.reply_to(
          message,
          "✅ Bot başarıyla yüklendi ve arka planda çalıştırıldı!\n\n📌"
...
def handle_all_text_inputs(message):
  ...
  if user_id == ADMIN_ID and message.reply_to_message:
    ...
    if match:
      ...
      try:
        bot.send_message(
            target_user_id, f"💬 **Yetkiliden Destek Yanıtı:**\n\n{text}"
        )
        bot.reply_to(
            message,
            f"✅ Yanıt `{target_user_id}` ID'li kullanıcıya iletildi.",
            parse_mode="Markdown",
        )
      except Exception as e:
  ...
  if user_id == ADMIN_ID and state == "ban":
    try:
      ...
      if target_id == ADMIN_ID:
        ...
        bot.reply_to(
            message,
            f"✅ `{target_id}` ID'li kullanıcı banlandı ve tüm botları"
            " durduruldu.",
            parse_mode="Markdown",
        )
    except ValueError:
    ...
    try:
      ...
      if target_id in banned_users:
        ...
        bot.reply_to(
            message,
            f"✅ `{target_id}` ID'li kullanıcının banı kaldırıldı.",
            parse_mode="Markdown",
        )
      else:
    ...
    ticket_markup.add(
        InlineKeyboardButton(
            "💬 Yanıtla", callback_data=f"reply_to_{user_id}"
        )
    )
    ticket_msg = (
        "🎫 **Yeni Destek Mesajı (Ticket)**\n\n👤 **Kullanıcı ID:**"
        f" `{user_id}`\n💬 **Mesaj:**\n{text}"
    )
    try:
      bot.send_message(
          ADMIN_ID, ticket_msg, reply_markup=ticket_markup, parse_mode="Markdown"
      )
...
def handle_callback(call):
  ...
  if data.startswith("reply_to_"):
    ...
    bot.send_message(
        user_id,
        f"✍️ `{target_uid}` ID'li kullanıcıya yanıtınızı yazın:",
        parse_mode="Markdown",
    )
    return
  ...
  if data == "check_sub":
    if check_subscription(user_id):
      ...
      bot.answer_callback_query(
          call.id, "❌ Henüz kanala abone değilsiniz!", show_alert=True
      )
    return
  ...
  if data == "faq_menu":
    faq_text = (
        ...
        " miyim?**\nC: Evet! İstediğiniz kadar `.py` uzantılı dosya yükleyerek"
        " aynı anda birden fazla botu aktif tutabilirsiniz.\n\n**S: Botlarımı"
        " nasıl yönetebilirim?**\nC: '📂 Botlarım / Yönet' butonuna basarak"
        " çalışan tüm botlarınızı listeleyebilir, dilediğinizi durdurabilir"
        " veya loglarını görebilirsiniz."
    )
    markup = InlineKeyboardMarkup()
    markup.add(
        InlineKeyboardButton("🔙 Ana Menüye Dön", callback_data="back_to_main_user")
  ...
  if data.startswith("log_bot_"):
    ...
    if script_id in user_scripts and isinstance(user_scripts[script_id], dict):
      ...
      if log_path and os.path.exists(log_path):
        ...
        bot.answer_callback_query(
            call.id, "❌ Log dosyası bulunamadı!", show_alert=True
        )
    else:
  ...
  if data.startswith("stop_bot_"):
    ...
    bot.answer_callback_query(
        call.id, "🗑️ Bot durduruldu ve silindi!", show_alert=True
    )

    user_scripts = user_data.get(str(user_id), {})
    if not user_scripts:
      bot.edit_message_text(
          "ℹ️ Şu anda aktif çalışan bir botunuz bulunmuyor.",
          call.message.chat.id,
          call.message.message_id,
  ...
  if data.startswith("admin_") or data == "toggle_maintenance" or data == "back_to_main":
    ...
    if data == "admin_panel":
      ...
      bot.answer_callback_query(call.id)
      bot.send_message(
          user_id, "🚫 Banlanacak kullanıcının **Telegram ID** değerini yazın:"
      )
      ...
      bot.send_message(
          user_id,
          f"✅ Banı kaldırılacak ID'yi yazın:\nBanlılar: {banned_users}",
      )
  ...
  if not check_subscription(user_id):
    bot.answer_callback_query(
        call.id, "⚠️ Kanala abone olmalısınız!", show_alert=True
    )
    return
  ...
  if data == "add_file":
    bot.send_message(
        user_id,
        "📤 Çalıştırmak istediğiniz yeni `.py` dosyanızı buraya gönderin."
