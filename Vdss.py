...
API_KEY = '8961710042:AAGOY_JV4xK-wn1P9tp7o3FG-q0-dXoTmZQ' 
ADMIN_ID = 8687183701
...
def handle_callback(call):
    ...
    if data == "faq_menu":
        faq_text = (
            ...
            "C: '📂 Botlarım / Yönet' butonuna basarak çalışan tüm botlarınızı listeleyebilir, dilediğinizi durdurabilir veya loglarını görebilirsiniz."
        )
        markup = InlineKeyboardMarkup()
    ...
    if data.startswith("log_bot_"):
        ...
        if script_id in user_scripts:
            ...
            if log_path and os.path.exists(log_path):
                ...
                bot.answer_callback_query(call.id)
                bot.send_message(user_id, f"📋 **Bot Logları ({user_scripts[script_id]['file_name']}):**\n\n```text\n{log_content}\n```", parse_mode="Markdown")
            else:
    ...
    if data.startswith("stop_bot_"):
        ...
        if not user_scripts:
            ...
            for sid, info in user_scripts.items():
                ...
                markup.add(
                    InlineKeyboardButton(f"📋 Log: {info['file_name']}", callback_data=f"log_bot_{sid}"),
                    InlineKeyboardButton(f"🗑️ Durdur", callback_data=f"stop_bot_{sid}")
                )
    ...
    if data == "add_file":
        ...
        if not user_scripts:
            ...
            for sid, info in user_scripts.items():
                ...
                markup.add(
                    InlineKeyboardButton(f"📋 Log: {info['file_name']}", callback_data=f"log_bot_{sid}"),
                    InlineKeyboardButton(f"🗑️ Durdur / Sil", callback_data=f"stop_bot_{sid}")
                )
            markup.add(InlineKeyboardButton("🔙 Ana Menü", callback_data="back_to_main_user"))
